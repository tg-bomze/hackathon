#from db_requests import setPosition, sendMessage
				...
				if rows[i][5] != '': # если не пустое поле текст
					if table_row[0][4] != rows[i][4]: # если языки разные
						...
						if datetime.now() <= end_time: # если временные лимиты кончились
							...
							print(mes)
				response = '{"id":'+str(rows[i][0])+', "name":"'+rows[i][1]+'", "status":'+str(rows[i][2])+', "coord": "'+str(rows[i][3])+'", "text_orig":"'+rows[i][5]+'", "text_trans":"'+mes+'"}'
